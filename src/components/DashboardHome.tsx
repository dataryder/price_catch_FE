...
import { getWeeklyIndexCategory } from "../services/apiClient"; // Assuming this returns Promise<IndexData>
...
const HomeDashboard = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			const data = await getWeeklyIndexCategory();
			setCardData(data);
		};
		fetchData();
	}, []);
						...
						{listitemcategory.map((i_group) => (
							<CarouselItem key={i_group} className="pl-1 basis-1/2 md:basis-1/3 xl:basis-1/4">
								<div className="flex flex-col p-4 gap-2 border border-otl-gray-200 rounded-md mx-1">
									...
									{cardData && <IndexChart item_group={i_group} period="year" data={cardData} />}
									{/* TODO: SKELETON */}
								</div>
							</CarouselItem>
						))}
